...
export default function BottomNav() {
  ...
  return (
    <nav className="fixed bottom-0 left-0 right-0 z-50 bg-zinc-950/95 backdrop-blur-xl border-t border-zinc-800/80 pb-[env(safe-area-inset-bottom)] shadow-2xl">
        ...
        {TABS.map(({ id, label, Icon }) => {
          ...
          return (
            <button
              ...
              className={cn(
                "flex-1 flex flex-col items-center justify-center gap-1 text-[11px] font-bold transition-all duration-150 relative py-1 rounded-2xl active:scale-92 cursor-pointer",
                active
                  ? "text-orange-400"
            ...
            >
              {/* Active glow pill */}
              {active && (
                <span className="absolute top-1.5 w-8 h-1 rounded-full bg-orange-500 shadow-sm shadow-orange-500/50" />
              )}
              ...
              {isTraining && activeSession && !active && (
                <span className="absolute top-2.5 right-[calc(50%-14px)] w-2.5 h-2.5 rounded-full bg-orange-500 ring-2 ring-zinc-950 animate-pulse" />
              )}
            </button>
          );
        })}
