...
/**
 * React.memo: verhindert, dass jeder Tastendruck im Eingabefeld ALLE
 * Nachrichten neu durch react-markdown parst.
 */
      ...
      <div className={cn("flex flex-col gap-1.5 max-w-[88%] sm:max-w-[80%]", isUser && "items-end")}>
            ...
            <div className="space-y-2">
              <ReactMarkdown
                components={{
                  ...
                  strong: ({ node, ...props }) => <strong className="font-bold text-cyan-300 font-mono" {...props} />,
                  em: ({ node, ...props }) => <em className="italic text-zinc-300" {...props} />,
                  h1: ({ node, ...props }) => <h1 className="text-base sm:text-lg font-black mt-3 mb-2 text-white font-mono" {...props} />,
                  h2: ({ node, ...props }) => <h2 className="text-sm sm:text-base font-bold mt-3 mb-1.5 text-cyan-300 font-mono" {...props} />,
                  ...
                  table: ({ node, ...props }) => (
                    <div className="overflow-x-auto my-3 rounded-2xl border border-white/10 glass-panel">
                      <table className="w-full text-xs text-left border-collapse" {...props} />
                    </div>
                  ),
                  thead: ({ node, ...props }) => <thead className="bg-white/[0.05] text-zinc-200 font-bold border-b border-white/10" {...props} />,
                  th: ({ node, ...props }) => <th className="p-2.5 font-black uppercase text-[10px] tracking-wider text-cyan-400 font-mono" {...props} />,
        ...
        <div className="flex items-center gap-2 px-1">
          ...
          {message.model && !isUser && (
            <span className="text-[9px] px-2 py-0.5 rounded-md bg-white/[0.05] text-cyan-300 border border-white/5 font-mono">
              {message.model}
            </span>
          )}
        </div>
