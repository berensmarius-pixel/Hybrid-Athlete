...
//  - /api/*           → IMMER Netzwerk, kein Caching (auth + frische Daten)
...
self.addEventListener("fetch", (event) => {
  ...
  // Gehashte Build-Assets: cache-first mit Background-Update
  ...
  // Navigationen & Sonstiges: network-first, Fallback auf Cache/Offline-Shell
  ...
  // Übrige same-origin GETs (manifest, icons): stale-while-revalidate
  event.respondWith(
    caches.match(request).then((cached) => {
      const networkFetch = fetch(request)
        .then((response) => {
